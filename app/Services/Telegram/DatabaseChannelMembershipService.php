...
{
    private const MEMBER_STATUSES = ['creator', 'administrator', 'member'];
    ...
    public function check(TelegramUpdate $update): ChannelMembershipResult
    {
        ...
        foreach ($channels as $channel) {
            try {
                $response = BOT::getChatMember($channel->telegram_chat_id, (int) $userId);
    ...
    private function status(mixed $response): ?string
    {
        if (is_array($response)) {
            ...
            return is_array($result) && isset($result['status'])
