...
final class UserPortalController extends Controller
{
    ...
    public function shop(?Category $category = null)
    {
        ...
        $products = Product::query()
            ...
            ->with([
                'category',
                'plans' => fn ($query) => $query->active()->orderBy('sort_order'),
            ])
    ...
    public function wallet()
    {
        $wallet = Wallet::query()->firstOrCreate(
            ['user_id' => $this->user()->id, 'currency' => 'IRR'],
    ...
    {
        $account = ReferralAccount::query()->firstOrCreate(
            ['user_id' => $this->user()->id],
            ['code' => strtoupper('H' . substr(bin2hex(random_bytes(5)), 0, 8))]
        );
