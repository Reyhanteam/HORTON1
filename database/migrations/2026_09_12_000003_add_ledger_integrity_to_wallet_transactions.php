...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::table('wallets')->orderBy('id')->each(function (object $wallet): void {
            $previousHash = null;
            DB::table('wallet_transactions')->where('wallet_id', $wallet->id)->orderBy('id')->get()->each(function (object $transaction) use (&$previousHash): void {
                $metadata = $transaction->metadata ? json_decode($transaction->metadata, true, 512, JSON_THROW_ON_ERROR) : [];
                $createdAt = $transaction->created_at ? \Illuminate\Support\Carbon::parse($transaction->created_at)->format('Y-m-d H:i:s.u') : null;
                $payload = [
                ...
                DB::table('wallet_transactions')->where('id', $transaction->id)->update(['previous_hash' => $previousHash, 'ledger_hash' => $hash]);
                $previousHash = $hash;
            });
        });
    }
